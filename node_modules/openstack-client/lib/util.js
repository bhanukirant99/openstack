...
exports.transform = (body, resp) => {
    return {body, header: resp.headers}
}
...
exports.defres = function (service, path, singular, opts) {
    ...
    var run = function (token) {
        ...
        var result = util.${method}({
            ...
            transform: ${_transform} || util.transform,
        ${method !== "get" ? "    body: body" : ""}});

        ${_transform ? 'return result' : ''}
        return result.then(function({header, body}) {
            ${(action === "list" ? plural : _singular) ? `body = body && body["${action === "list" ? plural : _singular}"]` : ''}
            return (options && options.requestId) ? [header['x-openstack-request-id'] || header['x-compute-request-id'] || header['x-order-request-id'], body] : body
        })
    }
    if ("function" === typeof token.then) {
